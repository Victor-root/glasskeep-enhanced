...
MUTATIONS = [
    ...
    # Les neuf de robustesse.
    ("f1", "le refus JSON d'un identifiant de note déjà pris",
     "    if (existing) {\n      return res.status(409).json({ error: \"Note id already in use\" });\n    }",
     "    if (false) {\n      return res.status(409).json({ error: \"Note id already in use\" });\n    }"),
    ("f2", "le message lisible sur un corps illisible",
     "  if (err.type === \"entity.parse.failed\" || err instanceof SyntaxError) {",
     "  if (false) {"),
    ("f2", "l'identifiant donné à une note qui n'en a pas",
     "        const idPropose = n.id === undefined || n.id === null || n.id === \"\"\n          ? uid() : String(n.id);",
     "        const idPropose = String(n.id);"),
    ("f2", "le comptage à part des notes refusées à l'import",
     "          if (audioErr) { rejected++; continue; }",
     "          if (audioErr) { skipped++; continue; }"),
    ("f2", "le refus d'une date d'affichage qui n'en est pas une",
     ...
     "  const tsAffichage = normalizeDisplayTimestamp(body.timestamp);\n  if (false) return res.status(400).json({ error: tsAffichage.error });"),
    ("f2", "le signal envoyé aux autres appareils après un import",
     "    if (idsImportes.length > 0 || updated > 0) {", "    if (false) {"),
    ("f3", "le nettoyage de l'icône au retrait d'un collaborateur",
     "  deleteUserIconStmt.run(noteId, userIdToRemove);", "  void userIdToRemove;"),
    ("f3", "le refus explicite quand ce n'est pas votre note",
     "  if (note.user_id !== req.user.id) {\n    return res.status(403).json({ error: \"Only note owner can change collaborator access\" });\n  }",
     "  if (false) {\n    return res.status(403).json({ error: \"Only note owner can change collaborator access\" });\n  }"),
    ("f1", "la forme unique d'une note entre la liste et la lecture",
    ...
    ("f5", "la prise en compte du domaine déclaré depuis le panneau",
     ...
     '  if (false) {\n    return fromDeclaredId(declaredRpId, req, listenPort, "admin");\n  }',
     "server/services/webauthnRp.js"),
    ("f5", "l'avertissement donné à l'utilisateur avant qu'il essaie",
     "      available: webauthnRp.resolveRp(req).ok,", "      available: true,",
     "server/routes/passkeyRoutes.js"),
    ("f5", "la raison rendue à l'utilisateur au lieu d'un échec générique",
