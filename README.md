...
- **Card footer on closed notes** surfacing the tags directly on the card, plus a new optional **note "logo" image** — a small uploaded picture used purely as a visual identifier (e.g. dropping the Rustdesk logo on a Rustdesk note so it stands out in the grid). The logo lives in the card footer and is independent of the regular image attachments that already render inside the note body
