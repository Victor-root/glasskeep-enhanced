...
- 🪟 **QR sign-in is a sibling card, not a popup** — clicking "Sign in with a QR code" on the login screen now reveals a second `glass-card` next to the auth form on wide screens (`lg+`), or stacked below it on phones. A decorative `IconArrowBadgeRight` sits between the two cards on `lg+` so the visual flow reads "form → QR". An invisible mirror of the logo+title block sits at the top of the QR column so its card aligns pixel-for-pixel with the auth card regardless of locale or title length
- 📷 **QR scanner field of view widened** — the camera surface dropped from `aspect-square` (which forced `object-cover` to crop the portrait stream and roughly double the perceived zoom) to `aspect-[3/4]`, matching the back camera's native ratio. The visible area is now what the camera actually sees
...
- 🔗 **Header QR quick-access toggle** — opt-in switch in Settings → "Sign in another device" pins a QR icon left of the kebab in the notes header for one-tap access to the scanner. Persisted server-side via `/api/user/settings` so the preference follows the user across browsers. Off by default
...
- 🍔 **Kebab close-on-outside no longer opens the underlying note** — the previous fixed-inset-0 backdrop was clipped to the header's bounding rect (the header's permanent `transform: translateY(0)` for its slide animation turns it into the containing block for `position: fixed` descendants). Switched to a document-level `pointerdown` listener in capture phase plus a permanent click-capture listener that swallows the synthetic click after the menu state has flipped — so the note never receives the tap that dismissed the menu
...
- 📸 **QR scanner black screen on cold start** — `qr-scanner` detected our `visibility: hidden` on the `<video>` and overrode the element with `position: fixed; 1×1px; opacity:0` (Safari workaround it can't undo cleanly), so the feed painted into one invisible pixel forever. Replaced the visibility toggle with a plain `bg-black` overlay sitting on top of the video during the loading phase — qr-scanner has nothing to override and the camera lights up immediately
...
- 🎨 **Header QR icon vertical alignment** — using `<TI.Qrcode>` (a `<span display:inline-flex>` wrapper) shifted the glyph a few pixels above its Kebab / Sync siblings. Inlined the SVG directly so the baseline matches
