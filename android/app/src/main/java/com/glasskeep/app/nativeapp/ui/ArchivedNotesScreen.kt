...
/**
 * Read-only browsing of archived notes (GET /api/notes/archived), reusing
 * the main list's own NoteCard for identical rendering. Opening a card goes
 * through the normal NoteDetailScreen, which already has its own
 * Archive/Unarchive action, this screen's job is only to be a way in.
 *
 * Not backed by Room like the main list: that table only ever holds
 * non-archived notes (see NotesRepository.refresh()), so this fetches fresh
 * on entry and on a manual refresh instead of risking either wiping this
 * list on the next refresh() or leaking archived notes into the main grid.
 * One consequence worth knowing: unarchiving a note from its detail screen
 * and coming back here doesn't drop it automatically, tap Refresh.
 */
@Composable
fun ArchivedNotesScreen(
    ...
    val errorTemplate = stringResource(R.string.native_archived_error)
    ...
    fun refresh() {
        ...
        scope.launch {
            try {
                notes = repository.fetchArchivedNotes().map { it.toEntity() }
            } catch (t: Throwable) {
                NativeDebug.e("ArchivedNotesScreen refresh failed", t)
    ...
    Box(Modifier.fillMaxSize().then(bgModifier)) {
        Column(Modifier.fillMaxSize()) {
            Column {
                Row(
                    ...
                    Row(
                        ...
                        Text(
                            stringResource(R.string.native_archived_title),
                            color = titleColor,
            ...
            if (notes.isEmpty() && loading) {
                ...
                // add by also claiming the archive is empty.
                Box(Modifier.weight(1f).fillMaxWidth(), contentAlignment = Alignment.Center) {
                    Text(stringResource(R.string.native_archived_empty), color = subtextColor)
                }
