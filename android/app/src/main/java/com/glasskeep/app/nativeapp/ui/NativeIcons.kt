...
// The icons below back the rich-text toolbar (RichTextToolbar.kt). Same
...
// drawn at the toolbar's `.tabler-icon` stroke width, 1.75.

@Composable
fun BoldIcon(modifier: Modifier = Modifier, size: Dp = 24.dp, tint: Color = Color.Black) = TablerIcon(
    "M7 5h6a3.5 3.5 0 0 1 0 7h-6l0 -7 M13 12h1a3.5 3.5 0 0 1 0 7h-7v-7",
    modifier, size, tint,
)

@Composable
fun ItalicIcon(modifier: Modifier = Modifier, size: Dp = 24.dp, tint: Color = Color.Black) = TablerIcon(
    "M11 5l6 0 M7 19l6 0 M14 5l-4 14",
    modifier, size, tint,
)

/**
 * RichIcons.jsx's variant-aware Underline: Tabler's U-curve over a base
 * line drawn in the current underline [style] (simple, double, dotted,
 * dashed, wavy) and [lineColor], the curve itself in [tint].
 */
@Composable
fun UnderlineIcon(
    ...
    val curve = remember { PathParser().parsePathString("M7 5v5a5 5 0 0 0 10 0v-5").toPath() }
    val wave = remember { PathParser().parsePathString("M5 19 q2.33 -2 4.66 0 t4.66 0 t4.66 0").toPath() }
    Canvas(modifier.size(size)) {
        val scale = this.size.minDimension / 24f
        scale(scale, scale, pivot = Offset.Zero) {
            val stroke = Stroke(width = 1.75f, cap = StrokeCap.Round, join = StrokeJoin.Round)
            val line = lineColor ?: tint
            drawPath(curve, color = tint, style = stroke)
            when (style) {
                "wavy" -> drawPath(wave, color = line, style = stroke)
                "double" -> {
                    drawLine(line, Offset(5f, 18f), Offset(19f, 18f), strokeWidth = 1.75f, cap = StrokeCap.Round)
                    drawLine(line, Offset(5f, 21f), Offset(19f, 21f), strokeWidth = 1.75f, cap = StrokeCap.Round)
                }
                "dotted", "dashed" -> drawLine(
                    line, Offset(5f, 19f), Offset(19f, 19f), strokeWidth = 1.75f, cap = StrokeCap.Round,
                    pathEffect = PathEffect.dashPathEffect(if (style == "dotted") floatArrayOf(0.01f, 3f) else floatArrayOf(3f, 2f)),
                )
                else -> drawLine(line, Offset(5f, 19f), Offset(19f, 19f), strokeWidth = 1.75f, cap = StrokeCap.Round)
            }
        }
    }
}
...
@Composable
fun StrikeIcon(modifier: Modifier = Modifier, size: Dp = 24.dp, tint: Color = Color.Black) = TablerIcon(
    "M5 12l14 0 M16 6.5a4 2 0 0 0 -4 -1.5h-1a3.5 3.5 0 0 0 0 7h2a3.5 3.5 0 0 1 0 7h-1.5a4 2 0 0 1 -4 -1.5",
    modifier, size, tint,
)

@Composable
fun LinkIcon(modifier: Modifier = Modifier, size: Dp = 24.dp, tint: Color = Color.Black) = TablerIcon(
    "M9 15l6 -6 M11 6l.463 -.536a5 5 0 0 1 7.071 7.072l-.534 .464 " +
        "M13 18l-.397 .534a5.068 5.068 0 0 1 -7.127 0a4.972 4.972 0 0 1 0 -7.071l.524 -.463",
    modifier, size, tint,
)

/** tabler/external-link.svg: the link popover's "Open" button. */
@Composable
fun ExternalLinkIcon(modifier: Modifier = Modifier, size: Dp = 24.dp, tint: Color = Color.Black) = TablerIcon(
...
fun BulletListIcon(modifier: Modifier = Modifier, size: Dp = 24.dp, tint: Color = Color.Black) {
    // tabler/list.svg: 3 lines plus 3 dots. Tabler draws each dot as a
    // zero-length round-capped line ("M5 6l0 .01"), which doesn't reliably
    // rasterize through PathParser + Stroke, so the dots are circles as wide
    // as the stroke.
    val path = remember {
    ...
    Canvas(modifier.size(size)) {
        ...
        scale(scale, scale, pivot = Offset.Zero) {
            ...
            for (y in floatArrayOf(6f, 12f, 18f)) drawCircle(color = tint, radius = 0.875f, center = Offset(5f, y))
        }
    }
}

@Composable
fun NumberedListIcon(modifier: Modifier = Modifier, size: Dp = 24.dp, tint: Color = Color.Black) = TablerIcon(
...
/** tabler/camera.svg. */
@Composable
fun CameraIcon(modifier: Modifier = Modifier, size: Dp = 24.dp, tint: Color = Color.Black, strokeWidth: Float = 1.75f) = TablerIcon(
...
/** tabler/terminal-2.svg. */
@Composable
fun Terminal2Icon(modifier: Modifier = Modifier, size: Dp = 24.dp, tint: Color = Color.Black) = TablerIcon(
    "M8 9l3 3l-3 3 M13 15l3 0 M3 4m0 2a2 2 0 0 1 2 -2h14a2 2 0 0 1 2 2v12a2 2 0 0 1 -2 2h-14a2 2 0 0 1 -2 -2z",
    modifier, size, tint,
)

/** tabler/brand-github.svg. */
@Composable
fun BrandGithubIcon(modifier: Modifier = Modifier, size: Dp = 24.dp, tint: Color = Color.Black) = TablerIcon(
    "M9 19c-4.3 1.4 -4.3 -2.5 -6 -3m12 5v-3.5c0 -1 .1 -1.4 -.5 -2c2.8 -.3 5.5 -1.4 5.5 -6a4.6 4.6 0 0 0 -1.3 -3.2" +
        "a4.2 4.2 0 0 0 -.1 -3.2s-1.1 -.3 -3.5 1.3a12.3 12.3 0 0 0 -6.2 0c-2.4 -1.6 -3.5 -1.3 -3.5 -1.3" +
        "a4.2 4.2 0 0 0 -.1 3.2a4.6 4.6 0 0 0 -1.3 3.2c0 4.6 2.7 5.7 5.5 6c-.6 .6 -.6 1.2 -.5 2v3.5",
    modifier, size, tint,
)

/** tabler/brand-docker.svg. */
@Composable
fun BrandDockerIcon(modifier: Modifier = Modifier, size: Dp = 24.dp, tint: Color = Color.Black) = TablerIcon(
    "M22 12.54c-1.804 -.345 -2.701 -1.08 -3.523 -2.94c-.487 .696 -1.102 1.568 -.92 2.4c.028 .238 -.32 1 -.557 1h-14" +
        "c0 5.208 3.164 7 6.196 7c4.124 .022 7.828 -1.376 9.854 -5c1.146 -.101 2.296 -1.505 2.95 -2.46z " +
        "M5 10h3v3h-3z M8 10h3v3h-3z M11 10h3v3h-3z M8 7h3v3h-3z M11 7h3v3h-3z M11 4h3v3h-3z " +
        "M4.571 18c1.5 0 2.047 -.074 2.958 -.78 M10 16l0 .01",
    modifier, size, tint,
)

/** tabler/cpu.svg. */
@Composable
fun CpuIcon(modifier: Modifier = Modifier, size: Dp = 24.dp, tint: Color = Color.Black) = TablerIcon(
    "M5 5m0 1a1 1 0 0 1 1 -1h12a1 1 0 0 1 1 1v12a1 1 0 0 1 -1 1h-12a1 1 0 0 1 -1 -1z M9 9h6v6h-6z " +
        "M3 10h2 M3 14h2 M10 3v2 M14 3v2 M21 10h-2 M21 14h-2 M14 21v-2 M10 21v-2",
    modifier, size, tint,
)

/** tabler/arrows-down-up.svg, the update window's swap gauge. */
@Composable
fun ArrowsDownUpIcon(modifier: Modifier = Modifier, size: Dp = 24.dp, tint: Color = Color.Black) = TablerIcon(
    "M17 3l0 18 M13 7l4 -4l4 4 M7 21l0 -18 M3 17l4 4l4 -4",
    modifier, size, tint,
)

/** tabler/ram.svg: not a Tabler outline but a filled 512-unit memory
 *  stick, the letters RAM cut out of it, which its own inline style keeps
 *  filled under `.tabler-icon`. */
@Composable
fun RamIcon(modifier: Modifier = Modifier, size: Dp = 24.dp, tint: Color = Color.Black) {
    val path = remember {
        PathParser().parsePathString(RamPath).toPath().apply { fillType = PathFillType.EvenOdd }
    }
    Canvas(modifier.size(size)) {
        val scale = this.size.minDimension / 512f
        scale(scale, scale, pivot = Offset.Zero) {
            drawPath(path, color = tint)
        }
    }
...
private const val RamPath =
    "M106.672 42.432c0-9.958-8.074-18.032-18.022-18.032h-6.63c-9.948 0-18.022 8.074-18.022 18.032v49.878h42.674V42.432z" +
        "M192.003 42.432c0-9.958-8.074-18.032-18.023-18.032h-6.62c-9.94 0-18.013 8.074-18.013 18.032v49.878h42.657V42.432z" +
        "M277.332 42.432c0-9.958-8.074-18.032-18.022-18.032h-6.62c-9.94 0-18.013 8.074-18.013 18.032v49.878h42.656V42.432z" +
        "M362.672 42.432c0-9.958-8.074-18.032-18.014-18.032h-6.638c-9.94 0-18.014 8.074-18.014 18.032v49.878h42.666V42.432z" +
        "M63.998 469.569c0 9.958 8.074 18.031 18.022 18.031h6.63c9.949 0 18.022-8.074 18.022-18.031v-49.86H63.998V469.569z" +
        "M149.346 469.569c0 9.958 8.074 18.031 18.013 18.031h6.62c9.949 0 18.023-8.074 18.023-18.031v-49.86h-42.657V469.569z" +
        "M234.677 469.569c0 9.958 8.074 18.031 18.013 18.031h6.62c9.948 0 18.022-8.074 18.022-18.031v-49.86h-42.656V469.569z" +
        "M320.006 469.569c0 9.958 8.074 18.031 18.014 18.031h6.638c9.94 0 18.014-8.074 18.014-18.031v-49.86h-42.666V469.569z" +
        "M448.002 42.432c0-9.958-8.056-18.032-18.032-18.032h-6.62c-9.94 0-18.013 8.074-18.013 18.032v49.878h42.665V42.432z" +
        "M405.337 469.569c0 9.958 8.074 18.031 18.013 18.031h6.62c9.976 0 18.032-8.074 18.032-18.031v-49.86h-42.665V469.569z" +
        "M168.176 223.974h-19.206c-0.529 0-0.789 0.26-0.789 0.79v23.978c0 0.529 0.26 0.79 0.789 0.79h19.206" +
        "c8.209 0 13.77-5.033 13.77-12.712C181.946 229.007 176.384 223.974 168.176 223.974z" +
        "M250.536 232.317L238.48 267.286L262.845 267.286L250.932 232.317z" +
        "M0 118.971v274.076h512V118.971H0z" +
        "M198.38 300.002h-14.712c-1.058 0-1.578-0.395-1.982-1.328l-17.215-35.103h-15.502c-0.529 0-0.789 0.269-0.789 0.798" +
        "v34.304c0 0.798-0.538 1.328-1.328 1.328h-12.989c-0.79 0-1.319-0.529-1.319-1.328v-87.42c0-0.799 0.529-1.328 1.319-1.328" +
        "h34.842c16.561 0 28.742 10.998 28.742 26.895c0 11.788-6.754 20.929-17.08 24.768l18.937 36.69" +
        "C199.834 299.204 199.305 300.002 198.38 300.002z" +
        "M288.286 300.002h-12.99c-0.915 0-1.453-0.395-1.713-1.328l-5.966-17.485h-33.91l-5.956 17.485" +
        "c-0.269 0.932-0.79 1.328-1.722 1.328h-12.847c-0.932 0-1.201-0.529-0.932-1.328l30.859-87.42" +
        "c0.269-0.799 0.807-1.328 1.722-1.328h12.192c0.924 0 1.462 0.529 1.722 1.328l30.465 87.42" +
        "C289.47 299.473 289.21 300.002 288.286 300.002z" +
        "M381.771 298.675c0 0.798-0.53 1.328-1.328 1.328h-11.527c-0.79 0-1.319-0.529-1.319-1.328v-55.099h-0.529" +
        "l-19.072 42.388c-0.538 1.193-1.327 1.722-2.52 1.722h-7.688c-1.184 0-1.982-0.529-2.512-1.722l-19.071-42.388" +
        "h-0.529v55.099c0 0.798-0.53 1.328-1.328 1.328h-11.528c-0.789 0-1.318-0.529-1.318-1.328v-87.42" +
        "c0-0.799 0.529-1.328 1.318-1.328h11.788c1.202 0 1.867 0.404 2.261 1.328l24.768 55.502h0.529" +
        "l24.374-55.502c0.394-0.924 1.058-1.328 2.252-1.328h11.652c0.798 0 1.328 0.529 1.328 1.328V298.675z"
