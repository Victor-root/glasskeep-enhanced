...
// "This server's address" motif: the Tabler "world-www" globe, but with the
// three "www" strokes painted in the active shell-theme accent while the globe
// itself stays neutral (inherits the surrounding label's currentColor).
// Rendered inline — not via the shared .tabler-icon loader — precisely so the
// two parts can take different colours. Sized in px to match the 20px the
// panel's other section glyphs render at.
export function WorldWwwIcon({
  size = 20,
  className = "",
  wwwClassName = "text-[var(--gk-chrome-accent)]",
}) {
  return (
    <svg
      viewBox="0 0 24 24"
      ...
      strokeLinejoin="round"
    >
      {/* globe — inherits the label's colour */}
      ...
      {/* the three "www" strokes — painted in the active theme accent */}
      <g className={wwwClassName}>
