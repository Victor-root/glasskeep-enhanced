...
export default function MultiSelectToolbar({
  ...
  return (
        <header
          className="p-3 sm:p-4 flex items-center justify-between sticky top-0 z-[25] glass-card multi-select-bar mb-6"
          style={{
            position: "sticky",
        ...
        >
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {selectedIds.length === 2 && activeTagFilter !== "TRASHED" && onOpenSideBySide && (
              <button
                type="button"
                onClick={() => onOpenSideBySide(selectedIds)}
                className="px-3 py-1.5 rounded-lg font-semibold transition-all duration-200 bg-gradient-to-r from-indigo-500 to-violet-600 text-white hover:from-indigo-600 hover:to-violet-700 shadow-md shadow-indigo-300/40 dark:shadow-none hover:shadow-lg hover:shadow-indigo-300/50 dark:hover:shadow-none hover:scale-[1.03] active:scale-[0.98] btn-gradient text-sm"
              >
                {t("openSideBySide")}
              </button>
            )}
